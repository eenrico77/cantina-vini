...
export default function WishlistClientList({ items }: { items: any[] }) {
  if (items.length === 0) return <p className="text-gray-500 text-sm text-center py-4">Nessun vino desiderato per ora.</p>;
      ...
      {items.map(item => (
        <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm text-gray-500">{item.producer} {item.year ? `— ${item.year}` : ''}</p>
            <p className="text-xs text-gray-400 mt-1">
              {[item.color, item.region, item.country].filter(Boolean).join(" · ")}
            </p>
            {item.notes && <p className="text-sm mt-2 italic text-gray-600">"{item.notes}"</p>}
          </div>
