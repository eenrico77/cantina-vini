...
export default async function StatsPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto space-y-6 mb-20">
      <h1 className="text-3xl font-extrabold text-gray-900">Statistiche</h1>

      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
          <p className="text-xs text-gray-500 font-semibold mb-1 uppercase tracking-wider">In Cantina</p>
          <p className="text-3xl font-black">{totalBottles}</p>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
          <p className="text-xs text-gray-500 font-semibold mb-1 uppercase tracking-wider">Valore Stimato</p>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 col-span-2">
          <p className="text-xs text-gray-500 font-semibold mb-1 uppercase tracking-wider text-center">Bottiglie Bevute</p>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 space-y-2">
          {Object.entries(byColor).map(([color, qty]) => (
            <div key={color} className="flex justify-between items-center">
              ...
              <span className="text-sm font-bold bg-gray-100 px-2 py-0.5 rounded-full">{qty}</span>
            </div>
          ))}
          {Object.keys(byColor).length === 0 && <p className="text-sm text-gray-500">Nessun dato</p>}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 space-y-2">
          ...
          {Object.keys(byAging).length === 0 && <p className="text-sm text-gray-500">Nessun dato</p>}
        </div>
      </div>
    </div>
  );
